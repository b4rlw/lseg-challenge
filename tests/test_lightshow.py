"""
Module contains the functions which test lightshow objects.
"""
import os
import numpy as np
...
grid_dims = (1000, 1000)
test_instr = os.path.join("data", "test_input.txt")
real_instr = os.path.join("data", "coding_challenge_input.txt")


def test_valid_bounds():
    lgt = LightGrid(grid_dims, test_instr)
    lgr = LightGrid(grid_dims, real_instr)
    for obj in [lgt, lgr]:
        for _, bl_coord, tr_coord in obj.parse_instr():
            x1, y1 = bl_coord
            x2, y2 = tr_coord
            assert x2 <= x1
            assert y2 >= y1
...
def test_parse_instr():
    lg = LightGrid(grid_dims, test_instr)
...
def test_turn_on():
    part_1 = LightGrid((3, 3), test_instr)
    part_1.turn_on(*(2, 0), *(0, 2))
    assert np.array_equal(part_1.matrix, np.ones((3, 3)))

    part_2 = LightGridUpgraded((3, 3), test_instr)
...
def test_main():
    part_1 = LightGrid(grid_dims, test_instr)
    result_1 = part_1.main()
    assert result_1 == 998004

    part_2 = LightGridUpgraded(grid_dims, test_instr)
